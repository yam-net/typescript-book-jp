...
JavaScriptにおいて`var`の変数は関数スコープ(function scoped)です。これは変数が*ブロックスコープ(blocked scope)*である他の多くの言語(C#/Javaなど)とは異なります。ブロックスコープの考えをJavaScriptに持ってきた場合、下記のコードは`123`を表示すると思うでしょうが、代わりに`456`が表示されます。
